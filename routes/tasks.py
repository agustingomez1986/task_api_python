...
from crud import create_task, delete_task_by_title, update_task as update_task_crud, get_task_by_owner_id
...
@router.delete("/tasks")
def delete_task(task_title: str, db:Session = Depends(get_db), current_user:User = Depends(get_current_user)):
    deleted_task = delete_task_by_title(db, task_title, current_user.id)
    if not deleted_task:
